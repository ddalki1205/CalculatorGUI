...
layout = [('x^a', '**'), ('|÷|', '//'), ('%', '%'), ('C', 'C'), ('7', 7), ('8', 8), ('9', 9), ('÷', '/'), ('4', 4), ('5', 5), ('6', 6), ('*', '*'), ('1', 1), ('2', 2), ('3', 3), ('+', '+'), ('.', '.'), ('0', 0), ('=', '='), ('-', '-')]
win = Tk()
...
for (dv, av) in layout:
    Button(win, width = 5, height = 2, command = partial(cmd, av), text = dv, font = ("Century Gothic", 15), fg = 'white', bg = '#505050' if isinstance(av, int) or dv in '=.' else '#acacac' if dv != 'C' else '#FF9500').grid(row = row + 1, column = col)
    row = (row + 1) if col == 3 else row
    col = (col + 1) if col != 3 else 0
win.title("My Calculator") # <--- Edit pls
win.mainloop()